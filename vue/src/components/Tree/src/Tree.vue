  ...
  export default defineComponent({
    ...
    setup(props, { attrs, slots, emit, expose }) {
      ...
      function checkAll(checkAll: boolean) {
        ...
        if (checkAll && props.onCheckedAll) {
          props.onCheckedAll(state.checkedKeys);
        }
        if (!checkAll && props.onUncheckedAll) {
          props.onUncheckedAll(state.checkedKeys);
        }
      }
      ...
      const _height = props.height > 0 ? props.height : window.innerHeight - 156 - 38;
      const treeHeight = ref(_height);
      onMounted(() => {
        ...
        window.onresize = function () {
          treeHeight.value = props.height > 0 ? props.height : window.innerHeight - 156 - 38;
        };
      });
      watchEffect(() => {
      ...
      expose(instance);
      return () => {
        const { title, helpMessage, toolbar, search, checkable, strictly } = props;
        const showTitle = title || toolbar || search || slots.headerTitle;
        // const scrollStyle: CSSProperties = { height: 'calc(100% - 38px)' };
        const scrollStyle: CSSProperties = { height: treeHeight.value + 'px' };
        return (
