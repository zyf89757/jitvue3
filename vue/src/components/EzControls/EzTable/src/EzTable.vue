<template>
  <div>
    <EzToolBar v-if="showToolbar" :showLeftBorder="false">
      <div style="position: relative">
        <div class="ez-toolbar-left">
          <div class="ant-btn-group">
            ...
            <a-button v-if="title" :size="AppSize" type="text">{{ title }}</a-button>
            <div v-if="toolbar && toolbar.length > 0">
              <template v-for="item in toolbar" :key="item.name">
                <a-button
                  v-if="checked(item.auth) && item.visibled != false && item.type !== 'Upload'"
                  :size="AppSize"
                ...
                <BasicUpload
                  v-if="checked(item.auth) && item.type == 'Upload'"
                  :maxSize="20"
                  :btn-text="item.title"
                  :maxNumber="10"
        ...
        <div class="ez-toolbar-right">
          <div class="ant-btn-group">
            <a-button :size="AppSize" title="新增" @click="addRow" v-if="allowAdding">
              <svg
                t="1630847191422"
            ...
            <template v-if="editMode == 'batch'">
              <a-button :size="AppSize" title="保存" @click="saveEditData" :disabled="!CanSave">
                ...
                >
                  <path
                    d="M853.333333 0H768v426.666667H256V0H0v1024h1024V170.666667z"
                    p-id="25260"
                  />
            ...
            <a-button :size="AppSize" title="导出数据" @click="exportToExcel" v-if="allowExport">
              <svg
                t="1630846469620"
                class="ez-toolbar-icon"
                viewBox="0 0 1024 1024"
    ...
    <div class="table-container p-1.5 pt-0.5 pr-2">
      ...
      >
        ...
        />
        <template v-if="showMore && moreItems && moreItems.length > 0">
          <DxColumn
            :width="30"
            :allow-sorting="false"
            :allow-editing="false"
...
<script lang="ts" setup>
  ...
  const emits = defineEmits(['ezTable']);
  const props = defineProps({
    ...
    editMode: {
      type: String,
      validator: (value: string) => ['row', 'batch', 'popup'].includes(value),
      default: 'row'
    },
    allowAdding: {
      type: Boolean,
  ...
  function saveEditData() {
    ezTableGrid.value.instance.saveEditData();
    CanSave.value = false;
  }
  ...
  function cancelEditData() {
    ezTableGrid.value.instance.cancelEditData();
    CanSave.value = false;
  }
  ...
  function addRow() {
    if (props.formModeEvent) {
      ...
      ezTableGrid.value.instance.addRow();
    }
  }

  function refresh() {
    ezTableGrid.value.instance.refresh();
  }
  ...
  function showColumnChooser() {
    ezTableGrid.value.instance.showColumnChooser();
  }

  function exportToExcel() {
    ezTableGrid.value.instance.exportToExcel();
  }
  ...
  function toolbarClick(item) {
    if (item.click) item.click(ezTableGrid.value.instance, item);
  }
  ...
  onMounted(() => {
    emits('ezTable', ezTableGrid.value.instance);
  });
</script>
